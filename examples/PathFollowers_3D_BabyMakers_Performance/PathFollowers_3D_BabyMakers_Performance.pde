//--Path Following with behaviors and babymaking - performance optmization by using move(min steps before placing trail, max total trails)--
//--Spawn Babies using the 'b' key--
...
public void setup() {
  ...
  if (this.flockFlag) this.octree = Data.makeOctree(width, this);
...
public void draw() {
  ...
  // -----------------------CREEPER AGENTS---------------------------
  // -----------------------------------------------------------------
  if (this.flockFlag) Data.updateCreepersTree(this.creeperSet, this.showOctree);
  ...
  for (Creeper c : this.creeperSet) {
    ...
    if (c.behavior.isSeparateActive() && c.behavior.isInstanceable() && this.sepFlag) {
      c.behavior.creeperSeparate(8.0f, creeperSet);
    }
    ...
    if (this.flockFlag) {
      c.behavior.creeperflock(30.0f, 0.24f, 0.09f, 0.045f, 0.0f, creeperSet, Data.getOctree(), Data.getTreeNodes(), this.drawConn);
    }      
