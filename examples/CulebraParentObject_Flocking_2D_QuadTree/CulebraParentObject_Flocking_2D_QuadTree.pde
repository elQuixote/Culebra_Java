//--This example uses creeper objects but the behavior methods used accept references to culebra.objects.Object instead of creepers--
//ex. creeperFlock2D takes a list of creepers as an argument, while flock2D takes a list of culebra.objects.Object as an argument. so we create--
//a list of culebra.objects.Object and add the creepers to that. we can do that because of polymorphism.--see polymorphism examples.
...
Creeper creep;
List<culebra.objects.Object> creeperSet;
...
public void setup() {
  ...
  this.creeperSet = new ArrayList<culebra.objects.Object>();
...
public void draw() {
  ...
  Data.updateQTree(this.creeperSet, this.showQTree);
  // -----------------------------------------------------------------
  // -----------------------CREEPER AGENTS---------------------------
  // -----------------------------------------------------------------
  for (culebra.objects.Object d : this.creeperSet) {
    
    if(d instanceof Creeper){
      Creeper c = (Creeper)d; //downcasting to creeper from culebra.objects.Object - we can do that because Object is the Abstract class that creeeper inherits from.
    
      c.setMoveAttributes(2.5f, 0.0f, 1.5f);
      c.behavior.flock2DTree(50.0f, 0.2f, 0.09f, 0.045f, creeperSet, Data.getQuadtree(), Data.getQTNodes(), this.drawConn); //here we pass the list of objects instead of the list of creepers.
