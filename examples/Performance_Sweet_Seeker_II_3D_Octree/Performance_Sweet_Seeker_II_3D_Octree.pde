//--Performance is added by specifying in the move(minimum number of steps before laying trail position, max number of total trails)
...
public void draw() {
  ...
  for (Creeper c : this.creeperSet) {
    ...
    if (createTrails) {
      if (!(c instanceof BabyCreeper)) {  
        ...
        c.viz.drawGradientTrails(c.getTrailPoints(), 10, colorA, colorB, 255.0f, 3.0f);
        ...
        if (((BabyCreeper)c).getType() == "a") {
          ...
          c.viz.drawGradientTrails(c.getTrailPoints(), 20, colorB, colorA, 255.0f, 3.0f);
        } else {
          float colorA[] = new float[] {255.0f, 215.0f, 0.0f };
          float colorB[] = new float[] {  200f, 1f, 255f };
          c.viz.drawGradientTrails(c.getTrailPoints(), 20, colorA, colorB, 255.0f, 3.0f);
        }
      }
    }     
    pushStyle();
  ...
  for (culebra.objects.Object obj : this.seekerList) {
    ...
    if (this.chaseOtherTailFlag) {
      ...
      seek.behavior.trailFollowerBabyMaker(allCombinedTrails, 1500, 20, 10, this.triggerBabies, 2, true, this.creeperChildSpawners, this.creeperChildSpawnType); 
      ...
      seek.behavior.trailFollowerBabyMaker(allCombinedTrails, 1500, 20, 10, this.triggerBabies, 2, true, this.childSpawners, this.childSpawnType);
    }
    if (this.wanderFlag) {
      float change = 100;
    ...
    if (createTrails) {
      if (!(seek instanceof BabySeeker)) {  
        ...
        float colorA[] = new float[] { 1f, 0f, 0f };
        float colorB[] = new float[] { 0.73f, 0.84f, 0.15f };
        seek.viz.drawGradientTrails(seek.getTrailPoints(), 100, colorB, colorA, 255.0f, 3.0f);
        ...
        if (((BabySeeker)seek).getType() == "a") {
          ...
          seek.viz.drawGradientTrails(seek.getTrailPoints(), 100, colorA, colorB, 255.0f, 2.0f);
        } else {
